(() => {
  ...
  var ActivityGraph = class extends HTMLElement {
    constructor() {
      super();
      this.attrs = {};
      attributes.forEach(
      ...
      if (!this.attrs["range-end"]) {
        this.attrs["range-end"] = (/* @__PURE__ */ new Date()).toISOString().split("T")[0];
      }
      if (this.activityDataFromChildren.length > 0) {
        this.attrs["activity-data"] = this.activityDataFromChildren.join(",");
        this.setAttribute("activity-data", this.attrs["activity-data"]);
      }
      if (this.getAttribute("enhanced") === "\u2728")
        return;
      this.render();
    }
    ...
    get activityDataFromChildren() {
      return Array.from(this.querySelectorAll("[data-activity]")).map(
        (el) => el.getAttribute("data-activity")
      );
    }
  };
